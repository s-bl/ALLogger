...
class HDFWriter(AbstractWriter):
    def __init__(self, output_dir, min_time_diff_btw_disc_writes=10, filter='.*'):
    ...
    def write_to_disc(self, force=False):
        if self.min_time_diff_btw_disc_writes < self.min_time_diff_btw_disc_writes and not force:
            return
